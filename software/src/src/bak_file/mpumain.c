...
void uart_log_data_main(uint16_t log_data)
{
    USART_SendData(USART1, log_data);
    while(USART_GetFlagStatus(USART1, USART_FLAG_TC) == 0);
    USART_ClearFlag(USART1, USART_FLAG_TC);
}

void uart_log_number_main(uint16_t log_number)
{
    if (log_number < 0)
    {
        uart_log_data_main('-');
    ...
    if (log_number < 10)
    {
        uart_log_data_main((log_number % 10) + 0x30);
        return;
    }
    short tmpNum = log_number / 10;
    uart_log_number_main(tmpNum);
    uart_log_data_main((log_number % 10) + 0x30);
}
...
int main(void)
{
    ...
    uart_log_data_main(0x0d);
    uart_log_data_main(0x0a);

    uart_log_number_main(12345);
    uart_log_data_main('-');
    uart_log_data_main(0x0d);
    uart_log_data_main(0x0a);
    uart_log_number_main(0);
    uart_log_data_main('-');
    uart_log_data_main(0x0d);
    uart_log_data_main(0x0a);
    Init_MPU6050();         //初始化MP
    ...
    while (1)
    {
        ...
        // Send_data('X');            //发送X轴数
        uart_log_data_main('X');
        uart_log_data_main(':');
        uart_log_number_main(T_X);

        uart_log_data_main('|');
        uart_log_data_main('Y');
        uart_log_data_main(':');
        uart_log_number_main(T_Y);

        uart_log_data_main('|');
        uart_log_data_main('Z');
        uart_log_data_main(':');
        uart_log_number_main(T_Z);
